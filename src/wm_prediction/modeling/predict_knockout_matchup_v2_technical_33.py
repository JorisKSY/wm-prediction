...
def load_team_snapshot(team_names: list[str]) -> pd.DataFrame:
    query = """
    SELECT *
    FROM features.team_prediction_snapshot_v2_technical_33
...
def make_matchup_features(home: pd.Series, away: pd.Series) -> pd.DataFrame:
    row = {
        # Technical v2 knockout assumption: neutral World Cup match.
        "is_neutral": 1,
        "is_friendly": 0,
        "is_world_cup": 1,
    ...
    return pd.DataFrame([row], columns=V2_TECHNICAL_33_NO_MOMENTUM_FEATURES)
...
class KnockoutMatchupPredictorV2Technical33:
    """Reusable knockout predictor for v2 technical 33-feature dry run."""
    ...
    def __init__(self, team_names: list[str]) -> None:
        ...
        if missing_teams:
            raise ValueError(f"Missing teams in v2 technical prediction snapshot: {missing_teams}")

        self.home_model = make_xgb_best()
        self.away_model = make_xgb_best()

        self.home_model.fit(
            self.train[V2_TECHNICAL_33_NO_MOMENTUM_FEATURES],
        ...
        # Perf: cache lambda pair per ORDERED (home, away) matchup. Deterministic
        # because make_matchup_features + models depend only on the team pair.
        # NOT symmetric: (A,B) != (B,A). Lives across the whole Monte Carlo run
        # (predictor is built once in main), so repeated matchups across sims are
        # computed only once.
    ...
    def predict(self, home_team: str, away_team: str) -> dict:
        ...
        if cached is not None:
            ...
            missing_features = int(x[V2_TECHNICAL_33_NO_MOMENTUM_FEATURES].isna().sum().sum())
            if missing_features:
                raise ValueError(f"Matchup features contain {missing_features} missing values.")

            lambda_home = float(
                np.clip(
                    self.home_model.predict(x[V2_TECHNICAL_33_NO_MOMENTUM_FEATURES])[0],
                    0.01,
                    20.0,
                )
            )
            lambda_away = float(
                np.clip(
                    self.away_model.predict(x[V2_TECHNICAL_33_NO_MOMENTUM_FEATURES])[0],
                    0.01,
                    20.0,
                )
            )
        ...
        return {
            ...
            "is_technical_dry_run": True,
        }


def predict_knockout_matchup(home_team: str, away_team: str) -> dict:
    predictor = KnockoutMatchupPredictorV2Technical33([home_team, away_team])
...
def main() -> None:
    ...
    print("Knockout matchup prediction v2 technical 33 no momentum")
