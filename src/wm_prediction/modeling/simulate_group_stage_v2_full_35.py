...
MODEL_NAME = "group_stage_monte_carlo_v2_full_35"
PREDICTION_MODEL_NAME = "xgboost_v2_full_35"
...
def load_match_predictions() -> pd.DataFrame:
    ...
    FROM features.match_predictions_v2_full_35
    ...
    if predictions.empty:
        raise ValueError("No rows found in features.match_predictions_v2_full_35.")

    if predictions["is_technical_dry_run"].any():
        raise ValueError("Expected v2_full_35 prediction rows not to be marked as dry run.")
...
def run_monte_carlo(
    ...
    summary["is_technical_dry_run"] = False
    summary["dry_run_reason"] = None
    summary["created_at"] = pd.Timestamp.now("UTC")
...
def write_summary_to_db(summary: pd.DataFrame) -> None:
    ...
    with engine.begin() as conn:
        conn.execute(text("DROP TABLE IF EXISTS features.group_stage_simulation_summary_v2_full_35"))

    summary.to_sql(
        name="group_stage_simulation_summary_v2_full_35",
        con=engine,
        schema="features",
        if_exists="replace",
        index=False,
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_group_stage_sim_summary_v2_full_35_team
                    ON features.group_stage_simulation_summary_v2_full_35 (team)
                """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_group_stage_sim_summary_v2_full_35_group
                    ON features.group_stage_simulation_summary_v2_full_35 ("group")
                """
            )
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-db",
        action="store_true",
        help="Write summary to features.group_stage_simulation_summary_v2_full_35.",
    )
...
def main() -> None:
    ...
    print("World Cup group-stage Monte Carlo v2 full 35")
