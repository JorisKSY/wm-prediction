...
from wm_prediction.modeling.predict_knockout_matchup_v2_technical_33 import KnockoutMatchupPredictorV2Technical33
...
from wm_prediction.modeling.simulate_group_stage_v2_technical_33 import load_match_predictions
...
def simulate_knockout_match(
    ...
    away_team: str,
    rng: np.random.Generator,
    predictor: KnockoutMatchupPredictorV2Technical33,
) -> dict[str, object]:
...
def simulate_tournament(
    seed: int,
    predictor: KnockoutMatchupPredictorV2Technical33 | None = None,
    matches: pd.DataFrame | None = None,
    groups: dict[str, list[str]] | None = None,
    ...
    if predictor is None:
        predictor = KnockoutMatchupPredictorV2Technical33(sorted(group_table["team"].unique()))
...
def print_single_simulation(
    ...
    print("World Cup full tournament simulation v2 technical 33 no momentum")
    print("Seed:", seed)
...
def write_tournament_summary_to_db(
    ...
    out.insert(0, "feature_set_name", "v2_technical_33_no_momentum")
    out.insert(0, "is_technical_dry_run", True)
    ...
    table_name = "tournament_simulation_summary_v2_technical_33"
    ...
    with engine.begin() as conn:
        ...
        if exists:
            conn.execute(
                text(
                    """
                    DELETE FROM features.tournament_simulation_summary_v2_technical_33
        ...
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_tournament_sim_summary_v2_technical_33_run
                    ON features.tournament_simulation_summary_v2_technical_33 (simulation_run_id)
                """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_tournament_sim_summary_v2_technical_33_team
                    ON features.tournament_simulation_summary_v2_technical_33 (team)
                """
            )
        )
...
def print_multi_simulation_summary(
    ...
    print("World Cup full tournament Monte Carlo v2 technical 33 no momentum")
...
def main() -> None:
    ...
    predictor = KnockoutMatchupPredictorV2Technical33(team_names)
