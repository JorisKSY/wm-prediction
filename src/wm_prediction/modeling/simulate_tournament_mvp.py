...
from wm_prediction.modeling.predict_knockout_matchup_mvp import KnockoutMatchupPredictor
...
def simulate_group_stage(rng: np.random.Generator) -> pd.DataFrame:
...
def simulate_knockout_match(
    ...
    away_team: str,
    rng: np.random.Generator,
    predictor: KnockoutMatchupPredictor,
) -> dict[str, object]:
...
def simulate_tournament(
    seed: int,
    predictor: KnockoutMatchupPredictor | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    rng = np.random.default_rng(seed)

    group_table = simulate_group_stage(rng)
    ...
    if predictor is None:
        predictor = KnockoutMatchupPredictor(sorted(group_table["team"].unique()))
...
def print_single_simulation(
    ...
    print("World Cup full tournament simulation MVP")
    print("Seed:", seed)
...
def write_tournament_summary_to_db(
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                DELETE FROM features.tournament_simulation_summary_mvp_v1
        ...
        out.to_sql(
            name="tournament_simulation_summary_mvp_v1",
            con=conn,
            schema="features",
            if_exists="append",
            index=False,
...
def print_multi_simulation_summary(
    ...
    print("World Cup full tournament Monte Carlo MVP")
...
def main() -> None:
    ...
    predictor = KnockoutMatchupPredictor(team_names)
